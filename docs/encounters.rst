...
偶遇事件一般用于添加随机风味事件；或者用于覆盖对应位置类型的内置对话，以达成触发额外对话链的功能。

additionalPlaceTypeTrigger
---------------------------

该元素下的内容代表了该偶遇事件可能触发的位置类型。每一种类型都需要使用 ``placeType`` 元素包裹。在此处定义的位置类型对于其下定义的偶遇事件通用。
...
该元素下的内容即偶遇事件的具体定义。每一种偶遇事件都将由对应的 ``encounter`` 元素定义。可以定义多个 ``encounter`` 元素。
